...
export default function AdminUploadStock() {
  ...
  const handleUpload = async () => {
    ...
    console.log('[FRONTEND] Starting upload process for:', file.name);
    setLoading(true);
    
    try {
      const token = await getToken('admin_token');
      console.log('[FRONTEND] Retrieved token');
      ...
      xhr.open('POST', `${api.baseURL}/api/admin/salesman/bulk-stock`);
      xhr.setRequestHeader('Authorization', `Bearer ${token}`);
      
      console.log(`[FRONTEND] Opened XHR POST to ${api.baseURL}/api/admin/salesman/bulk-stock`);
