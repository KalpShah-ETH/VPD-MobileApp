...
export async function POST(request) {
  ...
  try {
    ...
    if (salesman && salesman.expoPushToken && Expo.isExpoPushToken(salesman.expoPushToken)) {
      try {
        await expo.sendPushNotificationsAsync([{
          to: salesman.expoPushToken,
          sound: 'default',
          title: 'New Order',
          body: `${dbRetailer.shopName} placed an order`,
          data: { orderId: orderResults[0]?.order.id }
        }]);
      } catch (err) {
